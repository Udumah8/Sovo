...
export class WalletRebalancer extends BotModule {
  ...
  async rebalanceWallets() {
    ...
    this.logger.info('Average active wallet balance', { balance: (avgBalance.div(new BN('1000000000')).toNumber()).toFixed(4) });

    let needsFunding = walletBalances.filter(wb => wb.balance.lt(this.config.minWalletBalance)).sort((a, b) => a.balance.cmp(b.balance));
    let hasSurplus = walletBalances.filter(wb => wb.balance.gt(this.config.targetWalletBalance.mul(new BN('3')).div(new BN('2')))).sort((a, b) => b.balance.cmp(a.balance));
    ...
    if (needsFunding.length > 0 && hasSurplus.length > 0) {
      ...
      for (const needy of needsFunding) {
        ...
        const neededAmount = this.config.targetWalletBalance.sub(needy.balance);
        const surplusProvider = hasSurplus[surplusIndex];

        if (surplusProvider.wallet.pubkey === needy.wallet.pubkey) continue;

        const availableSurplus = surplusProvider.balance.sub(this.config.targetWalletBalance);
        ...
        if (availableSurplus.gt(neededAmount)) {
          try {
            ...
            this.logger.info('P2P Transfer', {
              ...
              amount: (neededAmount.div(new BN('1000000000')).toNumber()).toFixed(4)
            });
        ...
        if (surplusProvider.balance.sub(this.config.targetWalletBalance).lt(this.config.dustThreshold)) {
          surplusIndex++;
        }
      }
    }
    ...
    const remainingSurplus = hasSurplus.filter(wb => wb.balance.gt(this.config.targetWalletBalance.mul(new BN('11')).div(new BN('10'))));
  ...
  async consolidateSurplusToSink(surplusWallets) {
    ...
    for (const { wallet, balance } of surplusWallets) {
      const surplus = balance.sub(this.config.targetWalletBalance);

      if (surplus.gt(this.config.dustThreshold)) {
        try {
          const randomPct = 0.90 + Math.random() * 0.08;
          const transferAmount = surplus.mul(new BN(Math.floor(randomPct * 100))).div(new BN(100));

          if (transferAmount.lt(this.config.dustThreshold)) continue;
          ...
          this.logger.info('Consolidated to sink', {
            wallet: wallet.name,
            amount: (transferAmount.div(new BN('1000000000')).toNumber()).toFixed(4)
          });
